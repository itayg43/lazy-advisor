...
// Internal shape — prefer `TurnHandlerOutput` / `InitHandlerOutput` at handler
// boundaries. Exported only so the runner can type its loop variable neutrally
// (it consumes both init and turn outputs uniformly). Discriminated on
// `HandlerOutputKind` so `state` is only required on the Ask path — a handler
// returning Done is ending the phase and has no successor state to hand back.
// The Done arm's `message` is optional: when set, the runner sends it (and
// pushes it onto history) before returning `result`; omit when the phase
// resolves without user-visible output — e.g., early skip from `initHandler`
// (T6's `bufferPercentage === 0` case).
...
// Aliases of `HandlerOutput` that document *where* the value comes from at
// handler signatures. The runner treats both uniformly via `HandlerOutput`.
export type TurnHandlerOutput<TState, TResult> = HandlerOutput<TState, TResult>;
export type InitHandlerOutput<TState, TResult> = HandlerOutput<TState, TResult>;
