...
All columns except `under 3 years` are **ranges**, not points. The agent picks a specific integer inside the cell based on qualitative signal (where the user sits within their risk bucket, how clean the timeline is). Buffer percentage is always `100 - equity`.

The `under 3 years` column is fixed at 0% across all rows. Money needed in under 3 years is dominated by the need for it to still be there — risk tolerance is not a meaningful dial at that horizon (Vanguard, Fidelity, Bogleheads). The agent always proposes 0% equity for this timeline regardless of risk tolerance. The user can still push back; see Rule 3.
...
- Any risk, under 3 years, ₪20,000 → propose 0/100 (fixed — short-horizon collapse, risk tolerance not a factor).
...
- Proposal 0/100 (any risk, under 3 years). User: "Make it all stocks." → sanity check: "Money you need in under 3 years usually isn't invested in stocks — a 30% drop right before you need it is hard to recover from. Still want to go that way?" User: "Yes." → extract 100/0.
