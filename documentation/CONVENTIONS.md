...
- **More than 3 domain params → group them into a named params object.** Give the bag a `PascalCase` `*Params` type (e.g. `ResolvePromptDecisionParams`) — unexported unless it travels across modules — so the signature reads as `(params: ResolvePromptDecisionParams)` and the fields are destructured in the body (per the rule below). Infrastructure dependencies (e.g., an API client) and identifiers stay positional — they are not counted. Contrast the 2-param swap-hazard case below, which stays an *inline* object type unless the pair travels through several functions
...
- **Annotate contract role and narrowed return together when both apply.** When a function implements a named contract type (e.g., `InitHandler<R>`, `TurnHandler<R>`) but its body only produces a narrower arm of that contract's return union, annotate both at the declaration site: `const initHandler: InitHandler<R> = async (): Promise<PromptDirective> => ({...})`. The contract annotation documents the function's role for the reader; the narrowed return documents the actual behavior. Omit the contract annotation only when the function is built by a factory whose return type already encodes the contract (e.g., `createTurnHandler` returns `TurnHandler<R>`)

## State & Ownership

Model state flow as ownership/borrowing (Rust-style) rather than shared mutable references. A function that receives a value it does not own must not mutate it — it returns a new value instead. This is how the codebase gets encapsulation guarantees without classes (see *Functions § DI via params*): mutation sites stay few and local, so every state change traces to one place per phase.

- **Borrow, don't mutate a shared reference.** A callee never mutates a value its caller passed. Phase turn handlers receive the current state and return the next state — as a patch the runner applies — instead of editing state in place. The single mutation site per phase is the runner's state replacement, not the handlers. Example: allocation's `createTurnHandler` returns patches; the counter is committed in one place.
- **Enforce the borrow at the cheapest level that's sound, chosen by lifetime.** Two cases:
  - _Owned-and-returned_ — the runner hands the value off and never reads the old object again (e.g. `TState`): a compile-time `DeepReadonly<TState>` reminder suffices, because there's no retained alias to corrupt at runtime.
  - _Retained-and-shared_ — the runner keeps and reuses the value across turns (e.g. `history`): a type can't protect it (`readonly` is erased and castable), so pass a `structuredClone` per use. The runtime copy is the guarantee; the `ReadonlyArray` type is only the reminder.
  - Reach for the runtime copy _only_ when the value is genuinely retained-and-shared — cloning an owned-and-returned value every call is waste. Example: `TurnHandler` in `run-conversation.types.ts`.
- **Narrow write capability with the type, not just convention.** When a given actor may write only part of a state object, express that structurally — e.g. a patch type `Partial<Omit<State, "fieldTheActorMustNotWrite">>` — so an illegal write fails type-check instead of relying on discipline. Example: `AllocationNegotiationStatePatch` structurally forbids a handler from writing the turn counter, which the runner owns.
