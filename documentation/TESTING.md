...
- **Scenarios**: come from the phase-specific rules file co-located with the implementation (e.g., `clarify.fields.rules.md`, `clarify.risk.rules.md`, `clarify.contribution.rules.md`) for phases with focused behavior, or the stage rules file in `workflow/stages/` for general stage behavior. One test case per rule.
