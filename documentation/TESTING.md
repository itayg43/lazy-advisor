...
- **Test case order mirrors rule order** for phase evals whose rules live in a single co-located file (fields, contribution, risk): tests are ordered by ascending rule number, with multiple tests for the same rule grouped together. When adding a new rule, insert its test at the matching position — do not append. Each intake handler (classify, out-of-scope, unrealistic, contradictory) lives in its own `intake/<handler>/` subfolder with its eval co-located.
