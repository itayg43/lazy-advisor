...
- **Multiple trials per eval deferred** — run history is the prerequisite; it will reveal whether flakiness is a real problem before investing in multi-trial averaging. Add trials only if `.runs.jsonl` shows consistent instability on a stable codebase.

### Tasks

| Task | Summary | Status |
|------|---------|--------|
| 12.1 | Eval run history — custom Vitest reporter writing `.runs.jsonl` per eval file | Completed |
