...
|------|------|-------|------------|
| 9.1 | API key auth (Bearer header or `?token=` query param for WS) | `src/server/middlewares/auth/auth.middleware.ts`, `src/server/config.ts` | 1.5 |
| 9.2 | Redis rate limiter (sliding window, N sessions/IP/hour) | `src/server/middlewares/rate-limiter/rate-limiter.middleware.ts` | 1.5 |
