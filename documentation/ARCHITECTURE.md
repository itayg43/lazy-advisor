...
`runConversation` is intentionally an outlier here: it has no exhaustion error of its own. Convergence is the handler's responsibility — handlers self-limit via a turn counter in the threaded state and return `Done` with the appropriate phase-result variant (`completed` or `unresolved`) on their own terms. The runner has no notion of a turn budget. Its only *defined* failure is `unresolved` (non-convergence); a `runConversation` phase (e.g. `collectAllocation`) therefore has no try/catch of its own — an infra/model throw that survives SDK retries (a post-retry OpenAI failure mid-negotiation) deliberately stays uncaught and propagates to the orchestrator boundary, which logs it once and converts it to `errored`/`SYSTEM_ERROR_EXIT_MESSAGE`. A phase-local catch would only re-log an error the service layer already logged and re-map an honest infra failure into a softer status.
