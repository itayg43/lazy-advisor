...
**Unresolved-split exit.** `collectAllocation` returns `{ status: "unresolved", reason: "allocation" }` if the user keeps counter-proposing past `ALLOCATION_MAX_NEGOTIATION_TURNS` without converging. The orchestrator dispatches a closing message rather than locking the user into a split they were still negotiating.
